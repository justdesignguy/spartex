...
@section('content')
    <div class="slim-pageheader">
        ...
        <h6 class="slim-pagetitle">Category List</h6>
    </div>
            ...
            <table id="dataTable" class="table display responsive nowrap">
                ...
                @foreach($categories as $item)
                    <tr>
                        <td>{{ $item->title }}</td>
                        <td>
                            <a href="{{ route('admin.category.edit', $item->id) }}" class="btn btn-info btn-sm" title="Edit">
                                <div><i class="fa fa-edit"></i></div>
                            </a>
                            <a href="javascript:;" onclick="confirmDelete('{{ route("admin.category.delete",$item->id) }}')" class="btn btn-danger btn-sm" title="Delete">
